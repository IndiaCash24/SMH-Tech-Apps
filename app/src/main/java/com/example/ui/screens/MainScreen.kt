...
fun MainScreen(
    ...
    Scaffold(
        ...
        when (selectedItem) {
            0 -> HomeScreen(viewModel, onNavigateToAppDetails, onNavigateToNotifications, { selectedItem = 1 }, modifier = Modifier.padding(innerPadding))
            1 -> AppsScreen(viewModel, onNavigateToAppDetails, onNavigateToBuy, modifier = Modifier.padding(innerPadding))
            2 -> PremiumScreen(viewModel, onNavigateToAppDetails, onNavigateToBuy, modifier = Modifier.padding(innerPadding))
            3 -> ProfileScreen(viewModel, onLogout, onNavigateToDebug, modifier = Modifier.padding(innerPadding))
        }
    }
}
